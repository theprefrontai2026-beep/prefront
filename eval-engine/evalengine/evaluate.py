...
from .contract import Finding, VersionStamp
from .family2 import evaluate_all as evaluate_family2
from .contract import CheckContext
from .provenance import build as build_provenance
from .reconstruct import reconstruct
from .visibility import VisibilityProfile


def version_key(binding: BindingProfile, visibility: VisibilityProfile) -> str:
    return f"{config.ENGINE_VERSION}:{binding.version}:{visibility.version}"


def evaluate_session(session_id: str, binding: BindingProfile, visibility: VisibilityProfile) -> list[Finding]:
...
def evaluate_and_persist(session_id: str, binding: BindingProfile, visibility: VisibilityProfile,
    ...
    vkey = version_key(binding, visibility)
    ...
    findings = evaluate_session(session_id, binding, visibility)
