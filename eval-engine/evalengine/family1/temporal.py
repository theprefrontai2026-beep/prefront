...
Only `automaton: {before: {intent}, requires_fact}` is implemented; there is
no `after` variant because nothing in prefront-check-families.md's examples
needs one and speculative syntax nobody can trigger isn't worth carrying.
...
def evaluate(session: Session, rule_pack: RulePack, ctx: CheckContext) -> list[Verdict]:
    ...
    for rule in rule_pack.by_engine("temporal"):
        ...
        target_intent = before.get("intent", "*")
        requires_fact = (rule.automaton or {}).get("requires_fact", "")
        if not requires_fact:
            continue
        for step in session.steps:
            if target_intent != "*" and step.intent != target_intent:
                continue
            established = any(
