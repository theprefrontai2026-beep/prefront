...
const PROVIDERS = ["anthropic/claude-3-5-sonnet-20241022", "openai/gpt-4o", "openai/gpt-4o-mini"];
...
export default function PolicyStudio({
  ...
  useEffect(() => {
    ...
    api.listRules(activeDocId).then(res => {
      const rs = Array.isArray(res) ? res : (res.rules || []);
      setRules(rs);
  ...
  async function handleUpload() {
    ...
    try {
      ...
      setUploadStatus(`Uploaded — ${doc.file_name}`);
    } catch (e: any) {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      const rs = Array.isArray(res) ? res : (res.rules || res.candidates || []);
  ...
  async function handleExtractAtoms() {
    ...
    try {
      const res = await api.extractAtoms(activeDocId, { provider });
  ...
  async function handleValidate() {
    ...
    try {
      ...
      setUnresolved(Array.isArray(uRes) ? uRes : (uRes.items || []));
            ...
            <div className="pf-panel">
              ...
              {tab === "rules" && (
                <>
                  ...
                  {summaryApproved > 0 && (
                    <div className="pf-panel" style={{ marginTop: 20, padding: "20px 24px" }}>
                      <h3>Publish policy</h3>
                      <p className="pf-hint">Publish all approved rules to the policy skills service.</p>
                      <div className="pf-publish-row">
                        <button className="pf-btn primary" onClick={handlePublish} disabled={publishBusy}>
