...
Governed decisions are persisted so the Dashboard and Decision Traces page read history from the DB rather than re-running the LLM on every load. Two paths in: `POST /api/decisions/refresh` runs the whole SecureBank catalog server-side (via `ORCHESTRATOR_URL`) and stores every result, and the Runtime tab best-effort `POST`s each interactive run (`RuntimeDiff.tsx`'s `persistDecision` — restored with the tab; for a while `POST /api/decisions` had no caller at all and the Decisions log could only be filled by the bulk refresh). All routes live in `prefront-ui/artifacts/api-server/src/routes/decisions.ts`.
