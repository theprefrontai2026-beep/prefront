...
def evaluate(session: Session, catalog: IntentCatalog, ctx: CheckContext) -> list[Verdict]:
    ...
    for step in session.steps:
        ...
        # Empty caller_role/channel means the binding never captured it (or,
        # for an inline caller, the deployment's identity source doesn't
        # carry a channel at all) - a data gap, not proof of violation, so it
        # fails OPEN rather than flagging every call. In practice this never
        # weakens real OOB detection: a real trace's user_role binding is
        # essentially always populated.
        role_ok = not entry.allowed_roles or not session.caller_role or session.caller_role in entry.allowed_roles
        channel_ok = not entry.allowed_channels or not session.channel or session.channel in entry.allowed_channels
