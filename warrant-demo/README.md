...
### Or as containers, with the PDS as a real service

```bash
docker compose -f ../docker-compose.warrant.yml up --build -d
open http://localhost:8140      # the console
...
identically either way, and `warrant-service/tests/test_demo_parity.py` asserts
that over a real socket, so the console doubles as a conformance test for the
service surface. The footer on the page says which mode produced the run.
...
| `remote.py` | the adapter for decisions over HTTP — no decision logic, just the same attributes backed by calls |
| `policy/action_registry.yaml` | Arcadia's verbs, as `warrant-service` loads them (generated by `gen_registry.py`) |
