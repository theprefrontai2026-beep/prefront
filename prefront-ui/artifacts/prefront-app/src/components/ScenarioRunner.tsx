/*
 * Scenario runner — a subject application's catalogue, run interactively.
 *
 * This is Verdict's SessionRunner, brought into the main app so the Runtime
 * tab holds the right surface for whichever demo is selected: SecureBank is
 * governed IN-BAND so its runtime story is the before/after diff
 * (RuntimeDiff.tsx), while LoanPro is ungoverned and observed OUT OF BAND, so
 * its story is a session and the findings the evaluator raised about it after
 * the fact. One tab, one question — "what does this application do at
 * runtime?" — answered in the form that application actually supports.
 *
 * Ported rather than shared: prefront-app and verdict have separate Docker
 * build contexts and no common stylesheet (see prefront-ui/CLAUDE.md), so a
 * workspace package would break both images until the lockfile were
 * regenerated. The standalone Verdict app still exists and still runs this
 * catalogue; a behavioural change made here must be made there too, by hand.
 *
 * Three adaptations from Verdict's copy, each marked ADAPTED below.
 */
import { useEffect, useMemo, useState } from "react";
// ADAPTED (1/3): the registry is reached through the same `@apps` alias
// here; `../demo` is Verdict's own re-export of it and does not exist in
// this app. SessionDetail comes from THIS app's Observability, which is a
// superset of Verdict's copy (the extra props are all optional).
import { orchestratorFor, type AppIdentity } from "@apps";
...
/** ADAPTED (2/3): unchanged, but load-bearing here for a reason Verdict
 *  does not have — this app keeps the tab MOUNTED across demo switches,
 *  so expand state keyed on a session id alone would carry across. The
 *  caller keys the whole component on the demo id to remount it. */
...
export default function ScenarioRunner({ app }: { app: AppIdentity }) {
